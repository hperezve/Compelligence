...
var updateComboPrimaryCompetitor = function(realvaluesCompetitorSent, realtextsCompetitorSent, idCmbPrimaryComp, question, winning) {
    ...
    if (realvaluesCompetitor != "") {
        for (i = 0; i < realvaluesCompetitor.length; i++) {
            ...
            if (winning != "X") {
                //var optionsw = $('#' + 'WinningCompetitor' + winning).attr('options');
                var optionsw = $('#' + 'WinningCompetitor' + winning)[0].options;
            }
    ...
    $.post(
            ...
            function(data) {
                if (data != null && data != '') {
                    results = data;
                    if (results != "") {
                        addCompetitorsToList(results, realvaluesCompetitor, realvaluesProduct, urlPofC, question);
                    }
                }
            });
    ...
    $.post(
            ...
            function(data) {
                if (data != null && data != '') {
                    results = data;
                    if (results != "") {
                        addProductsToList(results, realvaluesProduct, question);
                    }
                }
            });    
...
var showWinning = function(obj) {

    var sel = $(obj).val();
    if (sel == 6000) {
        $('#WinningCompetitorField').show();
    } else {
        $('#WinningCompetitorField').hide();
    }
};

function toggleCombo(obj) {
    if ($('#' + obj).is(':visible'))
        $('#' + obj).hide();
    else
        $('#' + obj).show();
...
function updateCombo(txt,list) {
    list = '#' + list;
    var filter = eval("/" + $(txt).val() + "/i");
...
function updateText(list,txt) {
    $('#' + txt).val($('#' + list + ' :selected').text());
    $('#' + list).hide();
};