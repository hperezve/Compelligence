...
$.widget('ui.spinner', {
	...
	_draw: function() {
		...
		var uiSpinner = self.element
			...
			.prop('autocomplete', 'off') // switch off autocomplete in opera
			.wrap(self._uiSpinnerHtml())
			.parent()
				// add buttons
				.append(self._buttonHtml())
				// add aria properties
				.prop('aria-valuemin',options.min)
				.prop('aria-valuemax',options.max)
				.prop('aria-valuenow',options.value)
				// add behaviours
				.hover(function() {
		...
		this.element
			...
			.prop('id', function(){
				if (this.id) {
					uiSpinner.prop('id', 'ui-spinner-'+ this.id);
				}
			})
	...
	_value: function(newVal) {
		...
		this._format(newVal);
		this.uiSpinner && this.uiSpinner.prop('aria-valuenow', this.value());
	},
	_setData: function(key, value) {
		...
		if (/min|max|step/.test(key) && this.options[key] != null) {
			...
			if (this.element.prop(key)) {
				this.element.prop(key, value);
			}
			
			// add aria properties
			if (/min|max/.test(key)) {
				this.element.parent().prop('aria-value'+key, value);
			}
