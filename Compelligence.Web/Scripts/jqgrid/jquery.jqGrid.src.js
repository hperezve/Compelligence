...
setForceGridWidth: function(nwidth, shrink) {
    return this.each(function() {
        var $t = this, cw,
        ...
        if ($t.p.footerrow) $($t.grid.sDiv).css("width", Math.round(nwidth / 2) + "px");
        if (shrink === false && $t.p.forceFit == true) { $t.p.forceFit = false; }
        if (shrink === true) {
...
$.jgrid.extend({
	...
	GridUnload : function(){
		return this.each(function(){
			if ( !this.grid ) {return;}
			var defgrid = {id: $(this).prop('id'),cl: $(this).prop('class')};
			if (this.p.pager) {
				$(this.p.pager).empty().removeClass("ui-state-default ui-jqgrid-pager corner-bottom");
			}
			var newtable = document.createElement('table');
			$(newtable).prop({id:defgrid.id});
			...
			$.each($t.p.colModel,function(i,n){
				...
				if(this.search){
						...
						if(surl) {
							...
							$.ajax($.extend({
								...
								complete: function(res,status) {
									...
									$("select",self).prop({name:cm.index || cm.name, id: "gs_"+cm.name});
									if(soptions.attr) {$("select",self).prop(soptions.attr);}
							...
							if (oSv) {	
								...
								$(elem).prop({name:cm.index || cm.name, id: "gs_"+cm.name});
								var so, sv, ov;
								...
								if(soptions.attr) {$(elem).prop(soptions.attr);}
					...
					case 'text':
						...
						if(soptions.attr) {$("input",thd).prop(soptions.attr);}
...
open:function(s,t){var h=H[s],c=h.c,cc='.'+c.closeClass,z=(parseInt(h.w.css('z-index')));z=(z>0)?z:3000;var o=$('<div></div>').css({height:'100%',width:'100%',position:'fixed',left:0,top:0,'z-index':z-1,opacity:c.overlay/100});if(h.a)return F;h.t=t;h.a=true;h.w.css('z-index',z);
 ...
 if(c.ajax) {var r=c.target||h.w,u=c.ajax;r=(typeof r == 'string')?$(r,h.w):$(r);u=(u.substr(0,1) == '@')?$(t).prop(u.substring(1)):u;
		...
		if(typeof unformatFunc !== 'undefined' && $.isFunction(unformatFunc) ) {
			...
			switch(formatType) {
				...
				case 'checkbox':
					...
					ret = $('input',cellval).prop("checked") ? cbv[0] : cbv[1];
					break;
				case 'select' :
...
// Modal functions
	...
	closeModal : function(h) {
		h.w.hide().prop("aria-hidden","true");
		if(h.o) {h.o.remove();}
	},
	hideModal : function (selector,o) {
		...
		if ($.fn.jqm && o.jqm === true) {
			$(selector).prop("aria-hidden","true").jqmHide();
		} else {
			if(o.gb !== '') {
				try {$(".jqgrid-overlay:first",o.gb).hide();} catch (e){}
			}
			$(selector).hide().prop("aria-hidden","true");
		}
	},
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {		
		...
		css = $.extend({}, css || {});
		rtlsup = $(p.gbox).prop("dir") == "rtl" ? true : false;
		...
		$(mc).addClass("ui-jqdialog-content ui-widget-content").prop("id",aIDs.modalcontent);
		$(mc).append(content);
		mw.appendChild(mc);
		$(mw).prepend(mh);
		...
		.prop({tabIndex: "-1","role":"dialog","aria-labelledby":aIDs.modalhead,"aria-hidden":"true"});
	...
	viewModal : function (selector,o){
		...
		if ($.fn.jqm && o.jqm === true) {
			if(o.jqM) { $(selector).prop("aria-hidden","false").jqm(o).jqmShow(); }
			else {$(selector).prop("aria-hidden","false").jqmShow();}
		} else {
			if(o.gbox !== '') {
				$(".jqgrid-overlay:first",o.gbox).show();
				$(selector).data("gbox",o.gbox);
			}
			$(selector).show().prop("aria-hidden","false");
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		try {
			if($("#info_dialog").prop("aria-hidden") == "false") {
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			case "textarea" :
				...
				$(elem).prop({"role":"textbox","multiline":"true"});
			break;
			case "checkbox" : //what code for simple checkbox
				...
				if( !options.value ) {
					...
					elem.value = cbval[0];
					$(elem).prop("offval",cbval[1]);
				}
				setAttributes(elem, options, ['value']);
				options = bindEv(elem,options);
				$(elem).prop("role","checkbox");
			break;
			case "select" :
				...
				if(options.multiple===true) {
					...
					$(elem).prop("aria-multiselectable","true");
				} else { msl = false; }
			...
			case "custom" :
				...
				try {
					if($.isFunction(options.custom_element)) {
						...
						if(celm) {
							celm = $(celm).addClass("customelement").prop({id:options.id,name:options.name});
		...
		return this.each(function(){
			...
			function getFormData(){
				$("#"+frmtb+" > tbody > tr > td > .FormElement").each(function(i) {
					...
					if (celm.length) {
						var  elem = celm[0], nm = $(elem).prop('name');
						$.each($t.p.colModel, function(i,n){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if(tmp === "" && this.edittype == "checkbox") {tmp = $(elc).prop("offval");}
						...
						if ( trdata.length===0 ) {
							trdata = $("<tr "+dc+" rowpos='"+rp+"'></tr>").addClass("FormData").prop("id","tr_"+nm);
							$(trdata).append(tmpl);
							$(tb).append(trdata);
							trdata[0].rp = rp;
						}
							...
							if(opt.defaultValue ) {
								...
								if(fld[0].type=='checkbox') {
									...
									if(vlc.search(/(false|0|no|off|undefined)/i)<0 && vlc!=="") {
										...
										fld.prop({checked:"",defaultChecked:""});
									}
								} else {fld.val(vl); }
							} else {
								if( fld[0].type=='checkbox' ) {
						...
						switch (cm[i].edittype) {
							...
							case "checkbox":
								...
								if(cm[i].editoptions && cm[i].editoptions.value) {
									...
									if(cb[0] == tmp) {
										$("#"+nm,"#"+fmid).prop("checked",true);
										$("#"+nm,"#"+fmid).prop("defaultChecked",true); //ie
									} else {
										$("#"+nm,"#"+fmid).prop("checked",false);
										$("#"+nm,"#"+fmid).prop("defaultChecked",""); //ie
									}
								} else {
									tmp = tmp.toLowerCase();
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && viewfld) {
						...
						trdata = $(tb).find("tr[rowpos="+rp+"]");
						if ( trdata.length===0 ) {
							trdata = $("<tr "+dc+" rowpos='"+rp+"'></tr>").addClass("FormData").prop("id","trv_"+nm);
							$(trdata).append(tmpl);
							$(tb).append(trdata);
							trdata[0].rp = rp;
						}
						$("td:eq("+(cp-2)+")",trdata[0]).html('<b>'+ (typeof frmopt.label === 'undefined' ? obj.p.colNames[i]: frmopt.label)+'</b>');
						$("td:eq("+(cp-1)+")",trdata[0]).append("<span>"+tmp+"</span>").prop("id","v_"+nm);
						if(setme){
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				if($t.p.direction == "rtl") { $(navtbl).prop("dir","rtl").css("float","right"); }
				if (o.add) {
					pAdd = pAdd || {};
					...
					$(tbd,navtbl)
					.prop({"title":o.addtitle || "",id : pAdd.id || "add_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
				...
				if (o.edit) {
					...
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.prop({"title":o.edittitle || "",id: pEdit.id || "edit_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var sr = $t.p.selrow;
							if (sr) {
				...
				if (o.view) {
					...
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.prop({"title":o.viewtitle || "",id: pView.id || "view_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var sr = $t.p.selrow;
							if (sr) {
				...
				if (o.del) {
					...
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.prop({"title":o.deltitle || "",id: pDel.id || "del_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var dr;
							if($t.p.multiselect) {
				...
				if (o.search) {
					...
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.prop({"title":o.searchtitle  || "",id:pSearch.id || "search_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
				...
				if (o.refresh) {
					...
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.prop({"title":o.refreshtitle  || "",id: "refresh_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
				...
				if ( nm != 'cb' && nm != 'subgrid' && cm.editable===true && nm != 'rn' && !$(this).hasClass('not-editable-cell')) {
					switch (cm.edittype) {
						case "checkbox":
							...
							tmp[nm]=  $("input",this).prop("checked") ? cbv[0] : cbv[1]; 
							break;
						case 'text':
						case 'password':
						case 'textarea':
	...
	restoreRow : function(rowid, afterrestorefunc) {
		return this.each(function(){
			...
			if(fr >= 0) {
				...
				$(ind).prop("editable","0").unbind("keydown");
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if ($t.p.savedRow.length>0) {
				...
				window.setTimeout(function () { $("#"+$t.p.knv).prop("tabindex","-1").focus();},0);
			}
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$(cc).html("").append(elc).prop("tabindex","0");
			...
			if(fr !== null) {
				...
				switch (cm.edittype) {
					...
					case "checkbox":
						...
						v = $("#"+iRow+"_"+nmjq,$t.rows[iRow]).prop("checked") ? cbv[0] : cbv[1];
						v2=v;
						break;
					case "password":
					case "text":
...
addSubGrid : function( pos, sind ) {
	return this.each(function(){
		...
		var subGridXml = function(sjxml, sbid){
			...
			var pID = $("table:first",ts.grid.bDiv).prop("id")+"_";
		...
		this.each(function(){
			...
			if( tarspan.hasClass(minus) ) {
				if(grp.showSummaryOnHide && grp.groupSummary[0]) {
					...
					if(r){
						while(r) {
							if($(r).prop('id') ==uid+String(num) ) { break; }
							$(r).hide();
							r = r.nextSibling;
				}
					}
...
$.jgrid.extend({
    ...
    columnChooser : function(opts) {
        ...
        if (opts.caption) {
            selector.prop("title", opts.caption);
        }
        if (opts.classname) {
			...
			"drop" : function (opts) {
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						var accept = $(ui.draggable).prop("id");
...
jQuery(selector).each(function() {
	...
	jQuery('th', jQuery(this)).each(function() {
		if (colModel.length === 0 && selectable) {
			...
			colModel.push({
				name: jQuery(this).prop("id") || jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this).html())).split(' ').join('_'),
				index: jQuery(this).prop("id") || jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this).html())).split(' ').join('_'),
	...
	jQuery('tbody > tr', jQuery(this)).each(function() {
		...
		jQuery('td', jQuery(this)).each(function() {
			if (rowPos === 0 && selectable) {
				...
				var rowId = input.prop("value");
				rowIds.push(rowId || data.length);
				if (input.prop("checked")) {
					rowChecked.push(rowId);
				}
				row[colModel[rowPos].name] = input.prop("value");
			} else {
