    ...
    */
    ...
    $.fn.jqGrid = function(pin) {
		...
		addCell = function(rowId, cell, pos, irow, srvr) {
		    ...
		    var v2 = v;
		    var indStr = v2.indexOf("window.opener.parent");
			var indStrSendMail = v.indexOf("ReSendMail");
		    var indRef = v2.indexOf("<a"); ;
		    if (indStr == -1) {
		        if (v2.length > 90 && indRef == -1) {
		            v2 = v.substring(0, 90) + "...";
		        }
		    }
			if (indRef != -1 && indStrSendMail == -1)
			{
				v = v.replace(/<a/g, "<a target='_blank'");
				v2 = v2.replace(/<a/g, "<a target='_blank'");
			}
		    prp = formatCol(pos, irow);
		    return "<td role=\"gridcell\" " + prp + " title=\"" + $.jgrid.stripHtml(v) + "\">" + v2 + "</td>";
		},
		...
		updatepager = function(rn, dnd) {
		    ...
		    if (pgboxes) {
		        ...
		        if (ts.p.loadonce) {
		            ...
		            $(".selbox", pgboxes).prop("disabled", true);
		        } else {
		...
		td, ptr, tbody, imgs, iac = "", idc = "";
            ...
            if (this.p.multiselect) {
                ...
                $('#cb_' + $.jgrid.jqID(ts.p.id), this).bind('click', function() {
                    if (this.checked) {
                        $("[id^=jqg_]", ts.rows).prop("checked", true);
                        ...
                        $("[id^=jqg_]", ts.rows).prop("checked", false);
                        $(ts.rows).each(function(i) {
                            if (!$(this).hasClass("subgrid")) {
            ...
            $(ts).before(grid.hDiv).click(function(e) {
                ...
                if (cSel === true) {
                    if (ts.p.cellEdit === true) {
                        ...
                        if (ts.p.multiselect && ts.p.multiboxonly) {
                            if (scb) { $(ts).jqGrid("setSelection", ptr[0].id, true); }
                                ...
                                $(ts.p.selarrrow).each(function(i, n) {
                                    ...
                                    $("#jqg_" + $.jgrid.jqID(n), ind).prop("checked", false);
                                });
                                ts.p.selarrrow = [];
                        ...
                        if (e[ts.p.multikey]) {
                            $t.p.isPressedKey = true;
                            $(ts).jqGrid("setSelection", ptr[0].id, true);
                        } else {
                        if (ts.p.multiselect && scb) //jqGrid Update
                            {
                                //$t.p.isPressedKey = false;
                                scb = $("[id^=jqg_]", ptr).prop("checked");
                                //$("[id^=jqg_]", ptr).attr("checked", scb);
                                //$(ts).jqGrid("setSelection", ptr[0].id, scb);
                                $(ts).jqGrid("setSelection", ptr[0].id, !scb);
                            } /*else {
                                if ($t.p.selarrrow.length == 0) {
                                    $t.p.isPressedKey = false;
                                    $(ts).jqGrid("setSelection", ptr[0].id, true);
                                } else {
                                    $t.p.isPressedKey = false;
    ...
    $.jgrid.extend({
            ...
            return this.each(function() {
                ...
                if (!$t.p.multiselect) {
                    if (pt.className !== "ui-subgrid") {
                        if ($t.p.selrow) { $("tr#" + $.jgrid.jqID($t.p.selrow), $t.grid.bDiv).removeClass("ui-state-highlight").prop("aria-selected", "false"); }
                        $t.p.selrow = pt.id;
                        $(pt).addClass("ui-state-highlight").prop("aria-selected", "true");
                        if ($t.p.onSelectRow && onsr) { $t.p.onSelectRow.call($t, $t.p.selrow, true); }
                    }
                } else {
                    $t.p.selrow = pt.id;
                    ia = $.inArray($t.p.selrow, $t.p.selarrrow);
                    if (ia === -1) { //jqGrid Updated
                        //if ($t.p.isPressedKey == true) {
                            if (pt.className !== "ui-subgrid") { $(pt).addClass("ui-state-highlight").prop("aria-selected", "true"); }
                            stat = true;
                            $("#jqg_" + $.jgrid.jqID($t.p.selrow), $t.rows[pt.rowIndex]).prop("checked", stat);
        ...
        resetSelection: function() {
            return this.each(function() {
                ...
                if (!t.p.multiselect) {
                    if (t.p.selrow) {
                        $("tr#" + $.jgrid.jqID(t.p.selrow), t.grid.bDiv).removeClass("ui-state-highlight").prop("aria-selected", "false");
                        t.p.selrow = null;
                    }
                } else {
                    ...
                    $("#cb_" + $.jgrid.jqID(t.p.id), t.grid.hDiv).prop("checked", false);
                    t.p.selarrrow = [];
                }
                t.p.savedRow = [];
            });
                ...
                if (data) {
                    $(this.p.colModel).each(function(i) {
                        ...
                        if (data[nm] != undefined) {
                            ...
                            if (t.p.treeGrid === true && nm == t.p.ExpandColumn) {
                                $("td:eq(" + i + ") > span:first", ind).html(vl).prop("title", $.jgrid.stripHtml(vl));
                            } else {
                                $("td:eq(" + i + ")", ind).html(vl).prop("title", $.jgrid.stripHtml(vl));
                            }
                            success = true;
                        }
                    });
                ...
                $(this.p.colModel).each(function(i) {
                    ...
                    if (action == "set") {
                        if (data[nm] != undefined) {
                            ...
                            $("tr.footrow td:eq(" + i + ")", t.grid.sDiv).html(vl).prop("title", $.jgrid.stripHtml(vl));
                            success = true;
                        }
                    } else if (action == "get") {
        ...
        /*Receive individual widths*/

        /*end-Receive individual widths*/
                    ...
                    if (ind) {
                        ...
                        if (nData !== "") {
                            ...
                            if ($t.p.treeGrid && $(".tree-wrap", $(tcell)).length > 0)
                                $("span", $(tcell)).html(v).prop("title", $.jgrid.stripHtml(v));
                            else
                                $(tcell).html(v).prop("title", $.jgrid.stripHtml(v));
                        }
