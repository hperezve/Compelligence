...
(function($) {
    ...
    $.widget("ui.multiSelect", {
        ...
        _create: function() {
            ...
            this.selectedActions = $('<div id="' + this.id + 'selectedActions" class="actions ui-widget-header ui-helper-clearfix"><span class="count">0 ' + $.ui.multiSelect.locale.itemsCount + '</span><a id="' + this.id + '_RemoveAll" href="#" class="remove-all-by-id">' + $.ui.multiSelect.locale.removeAll + '</a></div>').appendTo(this.selectedContainer);
            ...
            this.availableActions = $('<div id="' + this.id + 'availableActions" class="actions ui-widget-header ui-helper-clearfix"><input type="text" class="search empty ui-widget-content ui-corner-all"/><a id="' + this.id + '_AddAll" href="#" class="add-all-by-id">' + $.ui.multiSelect.locale.addAll + '</a></div>').appendTo(this.availableContainer);
            ...
            this.availableList = $('<ul id="' + this.id + 'availableList" class="available connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function() { return false; }).appendTo(this.availableContainer);
            var that = this;
            ...
            console.log("Make selection sortable - start")
            ...
            console.log("Setting up live search - start")
            // set up livesearch
            ...
            console.log("Setting up live search - done")

            console.log("Setting up batch actions - start")
            ...
            $(".remove-all-by-id").click(function() {
                var srId = that.id;
                var lrId = this.id;
                if (lrId.indexOf(srId) == 0) {
                    that._populateLists(that.element.find('option').removeAttr('selected'));
                }
                return false;
            });
            ...
            $(".add-all-by-id").click(function() {
                var saId = that.id;
                var laId = this.id;
                if (laId.indexOf(saId) == 0) {
                    that._populateLists(that.element.find('option').attr('selected', 'selected'));
                }
                return false;
            });
            console.log("Setting up batch actions - done")
        ...
        _populateLists: function(options) {
            console.log("populate list - started");
            ...
            console.log("populate list - groups done");
            ...
            console.log("populate list - done");

            // update count
            console.log("update count - start");
            this._updateCount();
            console.log("update count - done");
            console.log("filter apply - start");
            //that._filter.apply(this.availableContainer.find('input.search'), [that.availableList]);
            console.log("filter apply - done");
        },
        ...
        _getOptionNode: function(option) {
            var tempo = option;
            option = $(option);
            var node = $('<li class="ui-state-default ui-element" title="' + option.text() + '"><span class="ui-icon"/>' + option.text() + '<input type="hidden" value="' + tempo.value + '" /><a href="#" class="action"><span class="ui-corner-all ui-icon"/></a></li>'); //.hide();
