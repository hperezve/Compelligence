							...
							if(t.p.jsonReader.repeatitems === true) {
								...
								for (var k=0;k<srow.length;k++) {
									...
									if(t.p.treeGrid===true && nm == t.p.ExpandColumn) {
									    $("td:eq(" + k + ") > span:first", ind).html(vl).prop("title", $.jgrid.stripHtml(vl));
									} else {
									$("td:eq(" + k + ")", ind).html(vl).prop("title", $.jgrid.stripHtml(vl)); 
									}
								}
								success = true;
								return true;
			...
			var formFill = function(){
				...
				$.each(self.p.filterModel,function(i,n){
					...
					case "select":
						if(this.surl) {
							...
							$(td).load(this.surl,function(){
								...
								$("select", this).prop({ name: $t.index || $t.name, id: "sg_" + $t.name });
								if ($t.sopt) $("select", this).prop($t.sopt);
							...
							if($t.sopt.value) {
								...
								$(elem).prop({ name: $t.index || $t.name, id: "sg_" + $t.name }).prop($t.sopt);
					...
					case 'text':
						...
						if ($t.sopt) $("input", td).prop($t.sopt);
						if(self.p.gridToolbar===true && $t.width) {
							if($.browser.msie) {
