...
var createModal = function(aIDs, content, p, insertSelector, posSelector, appendsel) {
	...
	rtlsup = jQuery(p.gbox).prop("dir") == "rtl" ? true : false;
	...
	jQuery(mc).addClass("ui-jqdialog-content ui-widget-content").prop("id", aIDs.modalcontent);
...
var viewModal = function (selector,o){
	...
	if (jQuery.fn.jqm && o.jqm == true) {
	    if (o.jqM) jQuery(selector).prop("aria-hidden", "false").jqm(o).jqmShow();
	    else jQuery(selector).prop("aria-hidden", "false").jqmShow();
	} else {
		if(o.gbox != '') {
			jQuery(".jqgrid-overlay:first",o.gbox).show();
			jQuery(selector).data("gbox",o.gbox);
		}
		jQuery(selector).show().prop("aria-hidden", "false");
...
var hideModal = function (selector,o) {
	...
	if (jQuery.fn.jqm && o.jqm === true) {
	    jQuery(selector).prop("aria-hidden", "true").jqmHide();
	} else {
		if(o.gb != '') {
			try {jQuery(".jqgrid-overlay:first",o.gb).hide();} catch (e){}
		}
		jQuery(selector).hide().prop("aria-hidden", "true");
	}
};
...
function info_dialog(caption, content,c_b, modalopt) {
	...
	try {
	    if (jQuery("#info_dialog").prop("aria-hidden") == "false")
	...
	switch (eltype)
	{
		...
		case "checkbox" : //what code for simple checkbox
			...
			if( !options.value ) {
				...
				elem.value = cbval[0];
				jQuery(elem).prop("offval", cbval[1]);
		...
		case "custom" :
			...
			try {
				if(jQuery.isFunction(options.custom_element)) {
					...
					if(celm) {
					    celm = jQuery(celm).addClass("customelement").prop({ id: options.id, name: options.name });
