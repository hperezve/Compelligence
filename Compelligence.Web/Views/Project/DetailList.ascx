...
<div id="<%= ViewData["Scope"] %>ProjectDetailDataListContent" class="absolute">
    <asp:Panel ID="ProjectDetailToolbarContent" runat="server" CssClass="buttonLink">
        <%= Html.SecurityButton(SecurityButtonType.Button, SecurityButtonAction.NewDetail, new { onClick = "javascript: newEntity('" + Url.Action("Create", "Project") + "', '" + ViewData["Scope"] + "', 'Project', '" + ViewData["BrowseDetailName"] + "', '" + ViewData["Container"] + "', '" + ViewData["HeaderType"] + "', '" + ViewData["DetailFilter"] + "');" })%>
